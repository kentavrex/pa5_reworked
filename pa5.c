...
int handle_started_case(struct Context *ctx, Message *msg) {
    if (ctx->num_started < ctx->children) {
        if (!ctx->rec_started[ctx->msg_sender]) {
            if (lamport_time < msg->s_header.s_local_time)
                lamport_time = msg->s_header.s_local_time;
...
int handle_done_case(struct Context *ctx, Message *msg) {
    if (ctx->num_done < ctx->children) {
        if (!ctx->rec_done[ctx->msg_sender]) {
            if (lamport_time < msg->s_header.s_local_time)
                lamport_time = msg->s_header.s_local_time;
...
int handle_cs_request_case(struct Context *ctx, Message *msg) {
    if (ctx->mutexl) {
        if (lamport_time < msg->s_header.s_local_time) lamport_time = msg->s_header.s_local_time;
        ++lamport_time;
        ++lamport_time;
...
void handle_message(struct Context *ctx, Message *msg) {
    switch (msg->s_header.s_type) {
        case STARTED:
            handle_started_case(ctx, msg);
            break;
        case DONE:
            handle_done_case(ctx, msg);
            break;
        case CS_REQUEST:
            handle_cs_request_case(ctx, msg);
            break;
        default:
            break;
    }
...
// Функция для обработки CS_REQUEST сообщения
int handle_cs_request(Message msg, Context *ctx) {
...
// Функция для обработки DONE сообщения
int handle_done(Message msg, Context *ctx) {
    if (ctx->num_done < ctx->children) {
        if (!ctx->rec_done[ctx->msg_sender]) {
            if (lamport_time < msg.s_header.s_local_time)
                lamport_time = msg.s_header.s_local_time;
		...
		while (active || ctx.num_done < ctx.children) {
			...
			switch (msg.s_header.s_type) {
				case STARTED:
					...
					break;
				case CS_REQUEST:
					if (handle_cs_request(msg, &ctx) != 0) {
						return 8;
					}
					break;
				case DONE:
					if (handle_done(msg, &ctx) != 0) {
						return 7;
					}
					break;
				default:
